...
$(function(){
	...
	if(denc && denc!=''){
		bottons=[{
			...
			callback:function(){
				...
				if(userInfo){
					var tnp = 'tn.' + userInfo.uid + '.mvt1.hpstar';
					var tnk = CryptoJS.SHA256(tnp+'.'+gid).toString();
				...
				window.location.href = 'http://trangnguyen.edu.vn';
			}
		},{
			caption:'Thoát',
			callback:function(){
	...
	var EndGame = function(data_score,key_store_score){
		if(test_id==3){
			...
			html = 'Tỗng thời gian: ' + total_time + '<br/>' + html;
			...
			if(total_score>150){
				bottons.push({
					caption:'Thi tiếp',
					callback:function(){
						window.location.href='../vong-' + (round_id + 1) + '/bai-1';
					}
				});
			...
			TN_POPUP.show(html,'Thông tin vòng thi ' + round_id,400,280,bottons,null,false);
			util.DelLocalStore(key_store_score);
		}
		else{
			var html = 'Bạn thi được: ' + data_score[test_id].score + ' điểm<br/>';
			html += 'Thời gian: ' + Second2Minute(data_score[test_id].time);
			var bottons = [
			{
				caption:'Thi tiếp',
				callback:function(){
					window.location.href='bai-' + (test_id+1);
				}
			},
			{
				caption:'Đáp án',
	...
	var NopBai = function(){
		...
		var key_store_score = CryptoJS.SHA256(type_id+'-'+round_id+'-'+window.userInfo.uid).toString();
