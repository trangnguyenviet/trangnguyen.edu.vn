$(function () {
			...
			if(g==undefined){
				Init_Game(function(game){
					...
					game.provide('submitAnswer', function (callback, answer, category) {
						...
						socket.emit('submitAnswer', {answer_index:answer.index,category_index:category.index},function(response){
	...
	function Init_Game(callback){
		...
		new Game({
			...
			message: {
				rule: 'Hãy kéo ô trống vào giỏ chủ đề, sao cho các nội dung tương đồng hoặc bằng nhau. Sai quá 3 lần bài thi kết thúc.'
			},
			sound: true,
			autoLoad: true,
			ready: function (game) {
