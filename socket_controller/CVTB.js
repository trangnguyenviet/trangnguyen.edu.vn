...
function SocketClient(io, socket, redis, DbUtil, process_send) {
		...
		async.waterfall([
			...
			function(data, callback) {
				DbUtil.GetExamInfo(type_id, user_info.class_id, round_id, test, function(err, exam_info) {
					if(err) {
						...
						if(exam_info) {
							//logger.error('exam_info',exam_info);

							let categories = exam_info.content;
							let answers = exam_info.answers;
							...
							for(let i = 0; i < categories.length; i++) {
								...
								let as = util.RandomArray(answers[i], number);
							...
							//logger.error('collection_answers',collection_answers);
							//logger.error('collection_index',collection_index);
							...
							if(questions != null) {
								let data_result = {};
								data.content = data_result.content = {
			...
			function(data, callback) {
				...
				if(test == 1) {
					...
					redis.hincrby(key_score_user, objKey.luot, 1, (err, result) => {
						data.luot = result;
						callback(false, data);
					}); //tăng lượt thi của vòng
				} else {
					callback(false, data);
				}
			},
			...
			function(data, callback) {
				...
				redis.hmset(key_user_play_exam,
					'type_id', type_id,
					...
					'wrongCount', 0,
					'currentIndex', 0,
					'time', data.exam_info.time,
					'user_exam_info', JSON.stringify(data.content)
					,function(err, result) {
					if(err) {
						response.error = 30000;
						response.message = 'Server đang bận, vui lòng thử lại sau';
	...
	socket.on('submitAnswer', function(data_req, result_callback) {
		if(util.isOnlyNumber(data_req.category_index) && util.isOnlyNumber(data_req.answer_index)) {
			let category_index = parseInt(data_req.category_index);
			let answer_index = parseInt(data_req.answer_index);
			if(category_index >= 0 && answer_index >= 0) {
				let response = {};
				async.waterfall([
					function(callback) {
						//SISMEMBER list_current_answers_<user_id> <answer_index>
						redis.sismember(key_list_current_answers, answer_index, function(err, is_exist) {
							if(err) {
								logger.error(err);
								response.error = 30000;
								response.message = 'Server đang bận, vui lòng thử lại sau';
					...
					function(data, callback) {
						...
						redis.lindex(key_list_correct_exam, answer_index, function(err, result) {
							if(err) {
								logger.error(err);
								response.error = 30000;
								response.message = 'Server đang bận, vui lòng thử lại sau';
								...
								if(result == category_index) {
									...
									redis.sadd(key_list_current_answers,answer_index,function(){
										redis.scard(key_list_current_answers,function(err,count){
											if(answers_count == count) {
												logger.trace('[CVTB] end game hết câu hỏi');
												end_game();
											}
										});
									});
									...
									redis.hincrby(key_user_play_exam, 'wrongCount', 1, function(err, err_count) {
										if(err_count >= 4) {
											logger.trace('[CVTB] end game sai 4');
											end_game();
										}
									});
								}
