...
router.get('/', function(req, res) {
	try{
		...
		if(config.isEvent16()){
			...
			param_render.title='Nhập mã thi Hương - Trạng Nguyên';

			if(req.tndata.user){
				// param_render.user = req.tndata.user;
				let key_get = req.tndata.code_exam_district;
				if(key_get && key_get!=''){
					...
					res.render('code_exam_district', param_render);
				}
			}
			else{
				//Bạn chưa đăng nhập
	...
	try{
		...
		async.waterfall([
			...
			(data,callback)=>{
				...
				.select('district_id class_id begin_use end_use')
				.exec((err,code_info)=>{
					if(err){
						...
						if(code_info){
							if(code_info.district_id == req.tndata.user.district_id){
								if(!code_info.class_id || code_info.class_id == 0 || code_info.class_id == req.tndata.user.class_id) {
									...
									if(timestamp>=code_info.begin_use){
										if(timestamp<=code_info.end_use){
											req.tndata.code_exam_district=data.code;
											message_out.error = 0;
											message_out.message = 'Nhập mã thành công, bạn có thể vào thi Hương';
										}
										else{
											message_out.error = 4;
