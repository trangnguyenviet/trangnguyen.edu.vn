...
router.get('/', function(req, res) {
	try{
		...
		//title
		param_render.title='Trò chơi - Trạng Nguyên';
		...
		async.parallel([
			...
			function(callback) {
				DbUtil.ListCategoryGame((err,list)=>{
					if(err){
						logger.error(err);
					}
					else{
		...
		function(list_err,list_result){
			res.render('list_game', param_render);
		});
	}
	catch(e){
...
router.post('/list/:category_id', function(req, res) {
	try{
		...
		if(category_id && util.isOnlyNumber(category_id)){
			...
			async.parallel([
				function(callback) {
					DbUtil.ListGame(category_id,page_size,page_index,(err,list)=>{
						if(err){
							logger.error(err);
						}
						else{
				...
				function(callback) {
					DbUtil.CountListGame(category_id,(err,iCount)=>{
						if(err){
							logger.error(err);
						}
						else{
...
router.post('/list-other', function(req, res) {
	try{
		...
		async.parallel([
			function(callback) {
				DbUtil.ListAllGame(page_size,page_index,(err,list)=>{
					if(err){
						logger.error(err);
					}
					else{
			...
			function(callback) {
				DbUtil.CountAllGame((err,iCount)=>{
					if(err){
						logger.error(err);
					}
					else{
...
router.get('/:name_ko_dau?.:id?', function(req, res) {
	try{
		...
		DbUtil.GameInfo(id,function(err,info){
			if(err){
				logger.error(err);
			}
			else{
				if(info){
					...
					res.render('game_detail', param_render);
				}
				else{
					res.sendStatus('404');
				}
	...
	try{
		...
		async.waterfall([
			...
			function(iCount,callback) {
				...
				DbUtil.ListRandomGame(page_size,skip,(err,list)=>{
					if(err){
						logger.error(err);
					}
					else{
